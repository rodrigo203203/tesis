...
Para la elección de la red neuronal se tomó en cuenta diferentes estudios en el cual comparan el rendimiento que tienen las diferentes redes neuronales convolucionales. El primer estudio realizado por un investigador de la empresa Face++ (\cite{estudio1}), realiza una prueba con varias redes neuronales para comparar el rendimiento entre detecciones de una sola etapa y detecciones en dos etapa. Los resultados de esta investigación están en la Tabla \ref{tab:investigacion1}. 

\begin{table}[h]
\caption{Comparación de rendimiento entre diferentes redes neuronales realizado por Face++.}
\centering
\begin{tabular}{ccc}
\textbf{Modelo} & \textbf{COCO (mP)} & \textbf{Tiempo (fps)}  \\ 
\hline
YOLO            & NA                 & NA                     \\
YOLOv2          & 21.6               & 40                     \\
SSD             & 25.1-28.8          & 46-19                  \\
...
RETINANET       & 39.1               & 5                     
\end{tabular}
\label{tab:investigacion1}
\vspace{3mm}
\sources{Gang Yu (Modificado).}
\end{table}

Otro estudio importante a tomar en cuenta es uno realizado por un equipo de investigadores de Google (\cite{google2}), donde el objetivo es ayudar a escoger una red neuronal brindando diferente información como la velocidad, precisión y consumo de recursos de las redes neuronales analizadas, el resumen de los resultados se encuentra en la Tabla \ref{tab:investigacion2}. 

Por último, se tomará en cuenta un estudio realizado por el equipo de YOLO (\cite{persona}), la cual pone a prueba la cuarta versión de su red neuronal y la compara con otras, el resultado de estas pruebas se puede observar en la Figura \ref{fig:labeling3}. 
\clearpage

\begin{table}[h]
\caption{Resultados de una prueba de rendimiento realizado investigadores de Google.}
\centering
\begin{tabular}{cc}
\textbf{Modelo}                                                   & \textbf{test-dev mAP}  \\ 
\hline
(Fastest) SSD w/MobileNet (Low Resolution)                        & 18.8                   \\
(Fastest) SSD w/Inception V2 (Low Resolution)                     & 21.6                   \\
(Sweet Spot) Faster R-CNN w/Resnet 101, 100 Proposals             & 31.9                   \\
(Sweet Spot) R-FCN w/Resnet 101, 300 Proposals                    & 30.3                   \\
(Most Accurate) Faster R-CNN w/Inception Resnet V2, 300 Proposals & 35.6                  
\end{tabular}
\label{tab:investigacion2}
\vspace{3mm}
\sources{Google Research (Modificado).}
\end{table}
...
Los datos a tomar en consideración son las únicas que fueron ejecutadas con la base de datos \say{COCO}, ya que es una recopilación de cientos de miles de imágenes lo cual facilita al momento de realizar pruebas en las diferentes redes neuronales y en los últimos años es la más usada para este tipo de pruebas. 

Estos resultados nos muestran que la cuarta versión de la red neuronal YOLO presenta una buena combinación entre precisión y velocidad, por otra parte, en el estudio realizado por Face++ nuevamente destaca YOLOv2 teniendo una buena combinación entre precisión y velocidad y en el estudio realizado por Google aunque no aparece YOLO ya que en ese momento la documentación que tenía YOLO no especificaba los datos suficientes como para poder realizar una comparación justa, pero por los resultados anteriores se puede considerar que YOLO se encontraría en la media de los datos que presenta la investigación.  
 Ya que se está buscando tener una red neuronal que tenga una buena precisión y que se ejecute de manera rápida, se optó por trabajar con la cuarta versión de YOLO ya que presenta un mejor resultado en estos aspectos.  
\clearpage
...
Este proceso se tiene que realizar con cada imagen, para así obtener un archivo que especifica la posición de dónde se encuentra la \say{persona} en la imagen. En la Figura \ref{fig:labeling2} se observa la manera en la que se guardan esos datos. De esta manera se obtiene una base de datos que sirve para entrenar la red neuronal YOLO, la cual debe estar compuesta por lo menos con 400 imágenes y para este entrenamiento se utilizo 500 imágenes que se obtuvieron mediante la extracción de fotogramas de un video de seguridad proporcionado por el supermercado Makro, la cual capta el sistema de fila única que posee este supermercado
...
Al momento de entrenar una red neuronal se utiliza muchos recursos de la computadora, sobre todo en el apartado del GPU, ya que la mayoría de los cálculos se realizan de mejor manera en esa parte. Esto resulto ser un problema ya que la computadora utilizada es una MacBook pro (2014) que no cuenta con una GPU dedicada y los entrenamientos se realizaban de manera muy lenta, ya que todos los cálculos serian realizados mediante el CPU de la computadora y este equipo cuenta con un procesador i5 de sexta generación, por ese motivo se optó por usar Colab, el cual es un servicio de Google que te permite realizar entrenamientos de redes neuronales ya que te da la opción de usar las GPU de sus servidores.  
\\

Las GPU que dispone  Colab cambian bastante, pero por lo general son la K80, la T4, la P4 y la P100 de NVIDIA. No hay una manera de elegir el tipo de GPU con la cual trabajar en su versión gratuita, pero pagando te da la opción a elegir. Colab es un editor de texto online, que te permite escribir tu código y probarlo sin la necesidad de instalar programas en tu computadora, pero será necesarios instalarlos en tu editor online. El primer paso es instalar Darknet, el cual te permite trabajar con YOLO.   
...
Antes de modificar los parámetros, se decidió realizar una prueba de rendimiento en consumo de recursos entre Tiny-Yolo y YOLO. Ya que Tiny-yolo al tener menos capas resulta ser una red más liviana, pero a costa de perder precisión. Por otra parte, YOLO al ser una red neuronal más compleja tiene una gran precisión, pero al costo de muchos recursos de la computadora. 
   
Para realizar esta prueba se analizó el mismo video con ambas redes neuronales para valorar la velocidad de ejecución, esta prueba fue realizada en una MacBook Pro (2014) que cuenta con un procesador Intel i5 de sexta generación y sin tarjeta de video dedicada. La diferencia en velocidad de ejecución se podrá ver de mejor manera en la Tabla \ref{tab:fruta}.
...
Estos resultados demuestran que la mejor opción en consumo de recursos es Tiny-Yolo, ya que se ejecuta mejor en máquinas que no poseen gran poder gráfico, pero las fallas en la detección de personas encontradas en las pruebas no compensan su buen rendimiento por lo cual se optó por trabajar con YOLO. Se modificara el archivo \say{yolov4-custom.cfg}, en el cual se cambiaran ciertos parámetros para que la red neuronal al momento de entrenar se adapte a los requerimientos del proyecto. los parámetros a modificar están señalados en la Figura \ref{fig:labeling6}. 
...
El siguiente es \textit{max batches}, el que se encarga de limitar el número de pasos máximos que realizará en el entrenamiento. Este número es obtenido mediante una simple fórmula que se observa en la Ecuación \ref{eq:maxb}, pero solo es aplicada cuando se trabajará con 3 o más clases de detección, caso contrario simplemente se tendrá que colocar \say{6000}, como es en este caso ya que solo se está trabajando con una clase, la cual es la detección de personas. En \textit{step} se tomará el 80\% y 90\% de \say{max batches} respectivamente. 
...
Para calcular la cantidad de filtros se hará uso de la Ecuación \ref{eq:clases}, el dato obtenido se tendrá que cambiar en todos los filtros que se encuentran en el documento \say{yolov4-custom.cfg}.
...
En este caso como solo se está trabajando con una sola clase (persona), el resultado obtenido mediante la fórmula es de 18 filtros. Con esto se finalizarán los cambios en el archivo \say{yolov4-custom.cfg}, pero aún faltaría editar otros 2 archivos más el primero definirá el nombre de la clase y el segundo definirá la ubicación de los archivos necesarios para entrenar, la ubicación de donde se guardará la red entrenada y cuantas clases se usará para entrenar, esto se aprecia en la Figura \ref{fig:labeling45}. 
...
Mediante Darknet se puede correr una función que pone a prueba la red neuronal para poder calcular la precisión media que tiene al momento de realizar las detecciones. El comando para poder realizar esto se observa en el Código. 

\begin{lstlisting}[language=Python,caption={Comando que realiza una prueba a la red neuronal para obtener la precisión media.},captionpos=b,label=23]
!./darknet detector map data/obj.data cfg/yolov4-custom.cfg /mydrive/yolov4-2/backup/yolov4-custom_last.weights
\end{lstlisting}
\vspace{5mm}

Con esta prueba se obtuvo una precisión media de 72.24\%, lo cual es un dato bastante bueno con el cual se puede garantizar una buena detección en la mayoría de los casos. Los resultados de esta prueba estarán en la parte de los anexos.


\section{Prueba de rendimiento}

Una vez entrenada la red neuronal se procede a realizar una prueba para poder calcular la precisión y el rendimiento que tiene al detectar personas. Para realizar esta prueba se está usando la documentación que proporciona la página de desarrolladores de Google (\cite{google}), donde se detalla las fórmulas para calcular la precisión.  
...
Una vez procesado el video por la red neuronal se extrajo 44 imágenes, con estas imágenes se procedió a realizar el análisis el cual permitiría medir la precisión de esta red neuronal. Para ellos las diferentes detecciones se dividió en cuatro categorías, estas son: verdadero positivo (VP), falso positivo (FP), falso negativo (FN) y verdadero negativo(VN). 

Para una detección sea considerado como VP, debe tener un 70\% de confiabilidad en la detección y también tiene que encerrar casi por completo al objeto detectado. Por otra parte, para que la detección sea considerada como FP debe tener una confiabilidad menor a 70\% o que enmarque una detección errónea, para que una detección pertenezca al grupo de FN el objeto no tiene que ser detectado y para que sea considerada en la ultima categoría la detección será considera VN no debe cometer una detección erronea. Esto se podrá entender mejor en la Figura \ref{fig:FN} 
...
Utilizando la anterior ecuación obtenemos una precisión del 71\%, la cual es bastante alto y demuestra una confiabilidad de los resultados que podrá obtener la red neuronal. Tomando en cuenta el resultado obtenido mediante el comando de Darknet, se obtuvo una precisión de 72.31\% es un dato no muy alejado al obtenido mediante un análisis manual, lo que valida estos resultados.  

 Pero para poder demostrar el rendimiento se tiene que obtener la curva ROC, para poder obtener esta gráfica será necesario obtener la tasa de verdaderos positivos y la tasa de falsos positivos. 
...
\begin{figure}[h]
	\centering
	\includegraphics[width=12cm,height=10cm]{imagenes/rocsi.png}
...
El valor obtenido de 0.63 puede mejorar realizando entrenamientos más largos para la red neuronal, otro valor a considerar es aumentar la cantidad de imágenes con lo que se realiza los entrenamientos para así dar más casos de estudio. 


Un factor importante para considerar es la posición de la cámara ya que si se encuentra en una posición donde no se pude captar bien la imagen esto ocasionaría problemas al momento de detectar a las personas.  En este caso el video donde se realizaron las pruebas cuenta con este problema, ya que en un punto dada la posición de la cámara no se pude observar bien a las personas, porque se posicionan una detrás de otra evitando que se pueda diferenciar bien. Esto se observa bien en la Figura \ref{fig:FROC2}. 

\begin{figure}[h]
	\centering
	\includegraphics[width=12cm,height=10cm]{imagenes/pruebared.png}
	\caption{Imagen del video donde se realizo las pruebas de rendimiento.}
	\source{Elaboración propia.}
	\label{fig:FROC2}
\end{figure}

La posición ideal de la cámara tendría que ser un lateral ya que desde esa posición es posible captar de buena manera a las diferentes personas que se encuentran haciendo fila como se ve en la Figura \ref{fig:FROC3}, de esta manera se evitaría el problema que se mencionó anteriormente. 
\clearpage
\begin{figure}[h]
	\centering
	\includegraphics[width=8cm,height=7cm]{imagenes/personas_fila.png}
	\caption{Vista lateral de una fila que podría mejorar el rendimiento de la red neuronal.}
	\source{Shutterstock.}
	\label{fig:FROC3}
\end{figure}
