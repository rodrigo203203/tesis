...
Antes de modificar los parámetros, se decidió realizar una prueba de rendimiento entre Tiny-Yolo y YOLO. Ya que Tiny-yolo al tener menos capas resulta ser una red mas liviana pero a costa de perder precisión. Por otra parte, YOLO al ser una red neuronal más compleja tiene una gran precisión pero al costo de muchos recursos de la computadora. 

La diferencia de rendimiento se podrá ver de mejor manera en la Tabla \ref{tab:fruta}, el cual da cómo mejor opción Tiny-Yolo teniendo un mejor rendimiento que al usar YOLO. 
...
Estos resultados demuestran que la mejor opción es trabajar con Tiny-Yolo, ya que corre mejor en maquinas que no poseen gran poder gráfico pero para no perder precisión en las detecciones se opto usar YOLO, por lo cual se modificara el archivo \say{yolov4-custom.cfg}, en el cual se cambiaran ciertos parámetros para que la red neuronal al momento de entrenar se adapte a los requerimientos del proyecto. los parámetros que se modificaron están señalados en la Figura \ref{fig:labeling6}. 
...
Para calcular la cantidad de filtros se hará uso de una fórmula simple que se puede apreciar en la Ecuación \ref{eq:clases}, este dato obtenido se tendrá cambiar en todos los filtros que se encuentra en el documento \say{yolov4-custom.cfg}.
...
Un factor impórtate a considerar es la posición de la cámara ya que si se encuentra en una posición donde no se pude captar bien la imagen esto ocasionaría problemas al momento de detectar a las personas.  En este caso el video donde se realizaron las pruebas cuenta con este problema, ya que en un punto dada la posición de la cámara no se pude observar bien a las personas, porque se posición una detrás de otra evitando que se diferencien bien. Esto se observa bien en la Figura \ref{fig:FROC2}. 
