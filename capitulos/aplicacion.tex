...
\begin{figure}[h]
	\centering
	\includegraphics[width=150mm, height=45mm]{imagenes/comando_ejecucion.png}
...
En este apartado se definen los valores de los parámetros necesarios para que la aplicación funcione. Como ser el caso del tiempo de espera máximo de una persona, ya que es un valor que se utiliza bastante en las funciones. Esto parámetros se observarán en el Código \ref{60}.
\vspace{5mm}
...
Con el anterior código se llega a saber cuál es la persona que esté al frente de la fila y con ellos determinar si se tarda mucho en ser atendido, ya que eso se sabe cuándo cruza el límite establecido. 

Los cálculos de estas funciones se reiniciarán cada vez que salte la alarma o cuando sea atendida una persona. 
...
Este registro de las alarmas se realiza con MongoDB, el cual lo guardara en este caso de manera local, y la forma en la guarda los datos se observan en la Tabla \ref{tab:fruta2}. 
...
En este apartado se desarrollará dos pruebas de la aplicación utilizando un video de seguridad proporcionado por Makro Parque, el cual capta a las personas haciendo fila. Los resultados que se obtendrán de esta prueba indicaran el tiempo promedio de espera que tienen los clientes y se sabrá si en algún momento se superó el tiempo máximo de espera.
 
El primer video que se analizara tiene una duración de 20 minutos y fue grabado un martes al medio día, se eligió esta hora ya que es considerada un periodo donde hay mucho flujo de personas en el local. 

Antes de poder ejecutar debidamente la aplicación se tiene que configurar los parámetros principales, el cual dependerá de los requerimientos del supermercado. En este caso, Makro Parque considera que una persona no puede esperar más de 6 minutos en ser atendido.  Otro factor importante es la posición de la cámara, que en este caso se encuentra posicionado en el techo teniendo una vista frontal y elevada de la fila, este dato es importante ya que ayuda a establecer las líneas que limiten la detección. Estas modificaciones se observan en el Código \ref{701}. 

\begin{lstlisting}[language=Python,caption={Configuración de los parámetros para el caso de estudio.},captionpos=b,label=701]
DETECTION_EVENTS = []
...
Una vez configurada la aplicación se procede a ejecutar la aplicación y la primera imagen que se observa son las líneas que indican los límites de detección, esto se podrá ver en la Figura \ref{fig:labeling80}.   
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.3]{imagenes/resultado-real.png}
	\caption{Imagen del video analizado para las pruebas donde se dibujan los límites de detección.}
	\source{Elaboración propia}
	\label{fig:labeling80}
\end{figure}

Una vez finalizado el video y analizando los resultados se puede concluir que la cantidad de cajas que utilizo el supermercado era el correcto ya que en ningún momento se activó la alarma y en cambio se obtuvo un buen tiempo promedio de espera ya que este resultado oscilaba entre un minuto a dos minutos de espera. 

El segundo video analizado tiene una duración de 27 minutos y fue grabado a las 7 de la noche, el cual es otra hora considerada donde existe mucho flujo de clientes. Igualmente, como la anterior prueba no se activa ninguna alarma y se obtiene un tiempo promedio de espera de veinte cinco a treinta segundos, lo cuales es un tiempo bastante bueno lo que demuestra que Makro Parque tiene un sistema de fila única muy eficiente. 
